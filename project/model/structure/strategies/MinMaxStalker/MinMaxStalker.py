...
class MinMaxStalker(Strategy, MyJson):
    ...
    def __init__(self, params: Map):
        ...
        self.__max_strategy = MinMaxStalker._CONST_MAX_STRATEGY
    ...
    def _set_next_stalk(self, unix_time: int) -> None:
        ...
        min_stalk_interval = MinMaxStalker.get_minimum_stalk_interval()
        stalk_interval = min_stalk_interval if (period is None) or (period <= min_stalk_interval) else period
        if MinMaxStalker._get_stage() == Config.STAGE_1:
            trade_index = MinMaxStalker._get_trade_index()
        ...
        """
        stalk_frequency = MinMaxStalker.get_stalk_frequency()
    ...
    def _add_fee(self, fee: Price) -> None:
        if fee.get_asset() != self.get_pair().get_right():
            ...
            raise ValueError(f"Fee must be in MinMaxStalker's '{right_asset}' right asset, instead '{fee}'")
        fees = self.get_fees()
        unix_time = _MF.get_timestamp(_MF.TIME_MILLISEC)
    ...
    def _set_next_blacklist_clean(self) -> None:
        ...
        """
        stalk_period = MinMaxStalker.get_stalk_period()
        if MinMaxStalker._get_stage() == Config.STAGE_1:
            trade_index = MinMaxStalker._get_trade_index()
    ...
    def get_blacklist(self) -> List[Pair]:
        ...
        unix_time = _MF.get_timestamp() if MinMaxStalker._get_stage() != Config.STAGE_1 else MinMaxStalker._get_trade_index()
    ...
    def _get_no_active_pairs(self, bkr: Broker) -> List[Pair]:
        allowed_pairs = MinMaxStalker._get_allowed_pairs(bkr)
        active_strategies = self.get_active_strategies()
        pair_strs = active_strategies.get_keys()
        blacklist = self.get_blacklist()
    ...
    def _launch_stalking(self, bkr: Broker) -> None:
        ...
        thread_name = _MF.generate_thread_name(MinMaxStalker._THREAD_BASE_MARKET_STALKING, 5)
    ...
    def _manage_stalking(self, broker: Broker) -> None:
        ...
        try:
            ...
            Bot.save_error(e, MinMaxStalker.__name__)
    ...
    def _stalk_market(self, bkr: Broker) -> None:
        ...
        _stg_cls = MinMaxStalker.__name__
        ...
        for pair_str, perf in perfs_sorted.items():
            ...
            if MinMaxStalker._eligible(market_price):
    ...
    def _set_market_analyse(self, bkr: Broker) -> None:
        _color_cyan = MinMaxStalker._TO_REMOVE_STYLE_CYAN
        _normal = MinMaxStalker._TO_REMOVE_STYLE_NORMAL
        _color_green = MinMaxStalker._TO_REMOVE_STYLE_GREEN
    ...
    def _bellow_floor(self, strategy: Strategy, market_price: MarketPrice, market_trend: str) -> bool:
        ...
        last_floor = MinMaxStalker._floor(last_roi, market_trend)
    ...
    def _floor(roi: float, market_trend: str) -> float:
        ...
        floor_coef = MinMaxStalker.get_floor_coef()
        min_floor_coef = MinMaxStalker.get_min_floor_coef()
    ...
    def _manage_trades(self, bkr: Broker) -> None:
        ...
        for pair_str, active_stg in active_stgs_copy.get_map().items():
            ...
            max_loss_rate = MinMaxStalker.get_max_loss()
        ...
        MinMaxStalker._save_moves(rows) if len(rows) > 0 else None
    ...
    def get_stalk_period() -> int:
        ...
        """
        return MinMaxStalker._CONST_MARKET_PERIOD

    @staticmethod
    def get_stalk_frequency() -> int:
        return MinMaxStalker._CONST_STALK_FREQUENCY

    @staticmethod
    def get_max_loss() -> float:
        return MinMaxStalker._CONST_MAX_LOSS_RATE

    @staticmethod
    def get_floor_coef() -> float:
        return MinMaxStalker._CONST_FLOOR_COEF

    @staticmethod
    def get_min_floor_coef() -> float:
        return MinMaxStalker._CONST_MIN_FLOOR_COEF
    ...
    def _get_market_price(bkr: Broker, pair: Pair) -> MarketPrice:
        ...
        mkt_params = Map({
            Map.pair: pair,
            Map.period: MinMaxStalker.get_stalk_period(),
            Map.begin_time: None,
            Map.end_time: None,
            Map.number: 100
        })
    ...
    def _get_allowed_pairs(bkr: Broker) -> List[Pair]:
        ...
        :return: Pair allowed to trade with this Strategy
                 Map[index{int}]:   {Pair}
        """
        if MinMaxStalker._CONST_ALLOWED_PAIRS is None:
            if MinMaxStalker._get_stage() == Config.STAGE_1:
            ...
            MinMaxStalker._CONST_ALLOWED_PAIRS = [Pair(pair_str) for pair_str in pair_strs]
        return MinMaxStalker._CONST_ALLOWED_PAIRS
    ...
    def get_bot_sleep_time() -> int:
        return MinMaxStalker._CONST_STALKER_BOT_SLEEP_TIME

    @staticmethod
    def generate_strategy(stg_class: str, params: Map) -> 'MinMaxStalker':
        """
        ...
        return MinMaxStalker(new_params)
    ...
    def _save_state(self, pair_to_closes: Map, to_delete_pairs: List[str], market_trend: str, market_analyse: Map) \
        ...
        next_stalk_date = _MF.unix_to_date(next_stalk) \
            if (MinMaxStalker._get_stage() != Config.STAGE_1) and (next_stalk is not None) else next_stalk
        lefts = []
        rights = []
    ...
    def _save_moves(rows: List[dict]) -> None:
        ...
        path = path.replace('$class', MinMaxStalker.__name__)
        fields = list(rows[0].keys())
