...
=======
                has_last_order = last_position is not None
                buy_datas[Map.buy] = int(last_position.get_buy_order().get_execution_time()/1000) if has_last_order else None
                buy_datas[Map.sell] = int(last_position.get_sell_order().get_execution_time()/1000) if has_last_order else None
...
=======
        last_buy_time =     datas[Map.buy] if datas[Map.buy] is not None else 0 # in second
        last_sell_time =    datas[Map.sell] if datas[Map.sell] is not None else 0 # in second
>>>>>>> Solomon-v5.3.1.12.2
        # Params
        now_index =     -1
        prev_index_2 =  -2
...
<<<<<<< HEAD
            ...
            {Map.callback: cls.is_market_trend_deep_and_rise,   Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_5min, marketprices=marketprices, index=prev_index_2, fall_ceiling_rate=SMT_RISE_CEILING, increase_rate=SMT_RISE_INCREASE, last_buy_time=last_buy_time, is_int_round=False)},
...
=======
=======
            {Map.callback: cls.has_execution_occurred_in_period,    Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, exec_time=last_sell_time)},
...
>>>>>>> Solomon-v5.4.4.3.3
=======
            {Map.callback: cls.compare_ema_and_keltner,         Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, index=prev_index_2, comparator='<', keltner_line=Map.low, ema_params=cls.EMA_PARAMS_2, keltner_params=cls.KELTNER_PARAMS_0)}
>>>>>>> Solomon-v5.4.4.3.6
=======
            {Map.callback: cls.is_tangent_macd_line_positive,   Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, index=prev_index_2, line_name=Map.histogram, macd_params=MarketPrice.MACD_PARAMS_1)},
            {Map.callback: cls.is_tangent_macd_line_positive,   Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, index=prev_index_3, line_name=Map.histogram, macd_params=MarketPrice.MACD_PARAMS_1)}
...
=======
        can_buy = cls.is_market_trend_bellow(**func_and_params[0][Map.param]) \
            and cls.is_tangent_market_trend_positive(**func_and_params[1][Map.param]) \
            and cls.is_keltner_roi_above_trigger(**func_and_params[2][Map.param]) \
            and cls.is_tangent_macd_line_positive(**func_and_params[3][Map.param]) \
            and cls.is_tangent_macd_line_positive(**func_and_params[4][Map.param]) \
>>>>>>> Solomon-v5.1.4.4.3
=======
        can_buy = cls.is_last_min_market_trend_bellow(**func_and_params[0][Map.param]) \
            and cls.is_tangent_market_trend_positive(**func_and_params[1][Map.param]) \
            and cls.is_keltner_roi_above_trigger(**func_and_params[2][Map.param]) \
            and cls.is_tangent_macd_line_positive(**func_and_params[3][Map.param]) \
            and cls.is_tangent_macd_line_positive(**func_and_params[4][Map.param]) \
...
>>>>>>> Solomon-v5.3.1.11.1.b
=======
        can_buy = not cls.has_execution_occurred_in_period(**func_and_params[0][Map.param]) \
            and cls.is_keltner_roi_above_trigger(**func_and_params[1][Map.param]) \
            and not has_bought_since_negative_tangent_ema(**func_and_params[2][Map.param]) \
            and cls.is_tangent_ema_positive(**func_and_params[3][Map.param])
        if can_buy:
            is_risky_zone = (
                                not cls.is_supertrend_rising(**func_and_params[4][Map.param]) \
                                or not cls.is_psar_rising(**func_and_params[5][Map.param])
                            )
            can_buy = cls.is_compare_price_and_keltner_line(**func_and_params[6][Map.param]) if is_risky_zone else can_buy
        if can_buy:
            superMACD = cls.is_supertrend_rising(**func_and_params[7][Map.param]), cls.is_macd_line_positive(**func_and_params[8][Map.param])
            if superMACD[0] and superMACD[1]:
                can_buy = can_buy \
                ...
                can_buy = can_buy \
                    ...
                    and cls.is_tangent_macd_line_positive(**func_and_params[12][Map.param]) \
                    and cls.is_tangent_macd_line_positive(**func_and_params[13][Map.param]) \
                    and cls.is_price_deep_enough(**func_and_params[14][Map.param]) \
                    and cls.is_tangent_ema_positive(**func_and_params[15][Map.param]) \
                    and cls.compare_exetrem_ema_and_keltner(**func_and_params[16][Map.param])
>>>>>>> Solomon-v5.3.1.12.2
=======
...
>>>>>>> Solomon-v5.4.4.2.2
        ...
        def keltner_roi_sell_price(vars_map: Map, broker: Broker, pair: Pair, period: int, marketprices: Map, index: int, keltner_line: str, buy_price: float, keltner_coef: float = 1, keltner_params: dict = {}) -> float:
            ...
            vars_map.put(keltner[prev_index],       Map.value,  f'{Map.key(Map.keltner, keltner_line, period_str, params_str)}[{prev_index}]')
            return sell_price
        def has_rsi_reached_trigger(vars_map: Map, broker: Broker, pair: Pair, period: int, marketprices: Map, index: int, buy_time: int, trigger: float, rsi_params: dict = {}) -> bool:
            period_str = broker.period_to_str(period)
            marketprice = cls._marketprice(broker, pair, period, marketprices)
            marketprice.reset_collections()
            marketprice_df = marketprice.to_pd()
            ...
            rsi.reverse()
            marketprice_df[Map.rsi] = pd.Series(rsi, index=marketprice_df.index)
            # Prepare
            rounded_buy_time = _MF.round_time(buy_time, period)
            time_at_index = marketprice_df[Map.time].iloc[index]
            trade_zone_df = marketprice_df[(marketprice_df[Map.time] >= rounded_buy_time) & (marketprice_df[Map.time] <= time_at_index)]
            above_trigger_df = trade_zone_df[trade_zone_df[Map.rsi] >= trigger]
            buy_date = _MF.unix_to_date(buy_time)
            date_start_trade_zone = _MF.unix_to_date(trade_zone_df[Map.time].iloc[0])
            date_end_trade_zone = _MF.unix_to_date(trade_zone_df[Map.time].iloc[-1])
            # Check
            prev_index = index - 1
            reached_trigger = above_trigger_df.shape[0] > 0
            # Put
            date_rsi_reach = _MF.unix_to_date(above_trigger_df[Map.time].iloc[0]) if reached_trigger else None
            rsi_param_str = _MF.param_to_str(rsi_params)
            k_base = f'has_rsi_reached_trigger_{period_str}_{rsi_param_str}[{index}]'
            vars_map.put(reached_trigger,       Map.condition,  k_base)
            vars_map.put(now_date,              Map.value,      f'{k_base}_now_date')
            vars_map.put(buy_date,              Map.value,      f'{k_base}_buy_date')
            vars_map.put(trigger,               Map.value,      f'{k_base}_trigger')
            vars_map.put(date_start_trade_zone, Map.value,      f'{k_base}_date_start_trade_zone')
            vars_map.put(date_end_trade_zone,   Map.value,      f'{k_base}_date_end_trade_zone')
...
=======
            ...
            # Put
            vars_map.put(can_stop,      Map.condition,  k_base)
            vars_map.put(keep_rate,     Map.value,      f'{k_base}_keep_rate')
            vars_map.put(keep_trigger,  Map.value,      f'{k_base}_keep_trigger')
            vars_map.put(buy_price,     Map.value,      f'{k_base}_buy_price')
            vars_map.put(max_price,     Map.value,      f'{k_base}_max_price')
            vars_map.put(max_roi,       Map.value,      f'{k_base}_max_roi')
            vars_map.put(buy_fee_rate,  Map.value,      f'{k_base}_buy_fee_rate')
            vars_map.put(sell_fee_rate, Map.value,      f'{k_base}_sell_fee_rate')
            vars_map.put(stop_price,    Map.value,      k_stop_price)
            return can_stop
>>>>>>> Solomon-v5.3.1.2.1
=======
        def compare_max_roi_and_trigger(vars_map: Map, comparator: str, buy_price: float, high_price: float, trigger: float) -> bool:
            # Prepare
            max_roi = _MF.progress_rate(high_price, buy_price)
            # Check
            compare = _MF.compare_first_and_second(comparator, max_roi, trigger)
            # Put
            prefix = f'compare_max_roi_{comparator}_{trigger}'
            vars_map.put(compare,       Map.condition,  prefix)
            vars_map.put(buy_price,     Map.value,      f'{prefix}_buy_price')
            vars_map.put(high_price,    Map.value,      f'{prefix}_high_price')
            vars_map.put(max_roi,       Map.value,      f'{prefix}_max_roi')
            return compare
>>>>>>> Solomon-v5.3.1.4.1
        vars_map = Map()
        period_1min = Broker.PERIOD_1MIN
...
>>>>>>> Solomon-v5.2
=======
        k_base = 'can_stop_losses'
        k_stop_price = f'{k_base}_stop_price'
        buy_price = datas[Map.buy]
...
=======
        ...
        buy_price =             datas[Map.buy]
        buy_fee_rate =          datas[Map.fee]
        position_higher_price = datas[Map.maximum]
        fees =                  broker.get_trade_fee(pair)
        maker_fee =             fees.get(Map.maker)
        trade_fees =            buy_fee_rate + maker_fee
>>>>>>> Solomon-v5.3.1.4.1
...
>>>>>>> Solomon-v5.2
=======
            {Map.callback: cls.has_execution_occurred_in_period,            Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, exec_time=buy_time)},
...
<<<<<<< HEAD
            ...
            {Map.callback: cls.is_supertrend_rising,            Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, index=now_index)},
            {Map.callback: can_stop_losses,                     Map.param: dict(vars_map=vars_map, keep_trigger=KEEP_TRIGGER_2, keep_rate=KEEP_RATE_2, buy_price=buy_price, max_price=position_max_price, buy_fee_rate=buy_fee_rate, sell_fee_rate=maker_fee)},
...
=======
            {Map.callback: cls.compare_keltner_floor_and_rate,  Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, index=now_index, comparator='<', keltner_line=Map.low, buy_price=buy_price, rate=trade_fees)},
            {Map.callback: cls.is_tangent_macd_line_positive,   Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, index=now_index, line_name=Map.macd)},
            {Map.callback: compare_max_roi_and_trigger,         Map.param: dict(vars_map=vars_map, comparator='>=', buy_price=buy_price, high_price=position_higher_price, trigger=MAX_ROI_TRIGGER)}
>>>>>>> Solomon-v5.3.1.4.1
=======
=======
            {Map.callback: cls.is_supertrend_rising,                        Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, index=now_index)},
...
>>>>>>> Solomon-v5.4.4.2.2
=======
            # {Map.callback: keltner_roi_sell_price,          Map.param: dict()}
            {Map.callback: has_market_trend_reach_max_drop, Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_5min, marketprices=marketprices, index=now_index, buy_time=buy_time, is_int_round=False)},
            {Map.callback: has_rsi_reached_trigger,         Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, index=now_index, buy_time=buy_time, trigger=RSI_TRIGGER)},
            {Map.callback: cls.is_tangent_ema_positive,     Map.param: dict(vars_map=vars_map, broker=broker, pair=pair, period=period_1min, marketprices=marketprices, index=now_index, ema_params=cls.EMA_PARAMS_1)}
>>>>>>> Solomon-v5.4.4.4.2
        ]
...
=======
        can_sell = has_switched_up_down(**func_and_params[0][Map.param])
=======
        can_sell = not cls.has_execution_occurred_in_period(**func_and_params[0][Map.param])
        if can_sell:
            if is_max_roi_reached(**func_and_params[1][Map.param]):
                can_sell = not cls.is_tangent_rsi_positive(**func_and_params[2][Map.param])
            elif has_macd_line_switched_positive(**func_and_params[3][Map.param]):
                can_sell = has_supertrend_switched_down(**func_and_params[4][Map.param]) \
                    or has_macd_line_switched_positive_then_negative(**func_and_params[5][Map.param]) \
                    and not cls.is_supertrend_rising(**func_and_params[6][Map.param])
            else:
                can_sell = not cls.is_tangent_ema_positive(**func_and_params[7][Map.param]) \
                    and not cls.is_tangent_macd_line_positive(**func_and_params[8][Map.param]) \
                    and cls.compare_keltner_floor_and_rate(**func_and_params[9][Map.param])
>>>>>>> Solomon-v5.3.1.12.2
=======
        if has_macd_line_switched_positive(**func_and_params[0][Map.param]):
            ...
            can_sell = not cls.is_tangent_ema_positive(**func_and_params[2][Map.param]) \
                and (
                        (
                            cls.compare_keltner_floor_and_rate(**func_and_params[3][Map.param]) \
                            and not cls.is_tangent_macd_line_positive(**func_and_params[4][Map.param])
                        ) \
                        or
                        compare_max_roi_and_trigger(**func_and_params[5][Map.param])
                    )
>>>>>>> Solomon-v5.3.1.4.1
=======
...
>>>>>>> Solomon-v5.3.1.7.1
=======
            is_less_risky_zone = cls.is_supertrend_rising(**func_and_params[9][Map.param]) \
                or cls.is_psar_rising(**func_and_params[10][Map.param])
            if is_less_risky_zone:
                can_sell = not cls.is_tangent_ema_positive(**func_and_params[11][Map.param]) \
                    and not cls.is_tangent_ema_positive(**func_and_params[12][Map.param])
            else:
                can_sell = not cls.is_tangent_ema_positive(**func_and_params[13][Map.param])
>>>>>>> Solomon-v5.3.1.7.2
        # Report
        report = cls._can_buy_sell_new_report(this_func, header_dict, can_sell, vars_map)
...
=======
        if has_macd_line_switched_positive(**func_and_params[0][Map.param]):
            ...
            can_sell = not cls.is_tangent_macd_line_positive(**func_and_params[2][Map.param]) \
                ...
                and not cls.is_supertrend_rising(**func_and_params[5][Map.param])
        # Stop Price
        can_stop_losses(**func_and_params[6][Map.param]) \
            or can_stop_losses(**func_and_params[7][Map.param])
        stop_price = vars_map.get(Map.value, k_stop_price)
        # Report
        report = cls._can_buy_sell_new_report(this_func, header_dict, can_sell, vars_map)
        return can_sell, report, stop_price
>>>>>>> Solomon-v5.3.1.2.1
=======
        vars_map.put(buy_case, Map.value, f'buy_case')
        can_sell = False
        ...
        can_sell = has_market_trend_reach_max_drop(**func_and_params[0][Map.param]) \
                or (
                    has_rsi_reached_trigger(**func_and_params[1][Map.param]) \
                    and not cls.is_tangent_ema_positive(**func_and_params[2][Map.param])
                )
        # Report
        report = cls._can_buy_sell_new_report(this_func, header_dict, can_sell, vars_map)
        return can_sell, report, sell_price
...
=======
    def has_execution_occurred_in_period(cls, vars_map: Map, broker: Broker, pair: Pair, period: int, marketprices: Map, exec_time: int) -> bool:
>>>>>>> Solomon-v5.3.1.12.2
        period_str = broker.period_to_str(period)
        marketprice = cls._marketprice(broker, pair, period, marketprices)
        marketprice.reset_collections()
...
=======
        # Check
        rounded_exec_time = _MF.round_time(exec_time, period)
        occurred_in_period = rounded_exec_time == now_time
        # Put
        k_base = f'has_execution_occurred_in_{period_str}'
        vars_map.put(occurred_in_period,                    Map.condition,  k_base)
        vars_map.put(_MF.unix_to_date(now_time),            Map.value,      f'{k_base}_date')
        vars_map.put(_MF.unix_to_date(exec_time),           Map.value,      f'{k_base}_exec_date')
        vars_map.put(_MF.unix_to_date(rounded_exec_time),   Map.value,      f'{k_base}_rounded_exec_date')
        return occurred_in_period
>>>>>>> Solomon-v5.3.1.12.2
...
>>>>>>> Solomon-v2.0.2.2
=======
            contain_trade = len(trades) > 0
            buy_datas[Map.buy] =    trades[-1][Map.buy][Map.time] if contain_trade else None
            buy_datas[Map.sell] =   trades[-1][Map.sell][Map.time] if contain_trade else None
...
=======
            ...
            sell_datas[Map.maximum] = trade[Map.maximum]
            sell_datas[Map.fee] = trade[Map.buy][Map.fee]
            can_sell, sell_condition, sell_stop_price = cls.can_sell(broker, pair, marketprices, sell_datas)
>>>>>>> Solomon-v5.3.1.2.1
            sell_condition = cls._backtest_condition_add_prefix(sell_condition, pair, marketprice)
            sell_conditions.append(sell_condition)
            #
            is_stop_set = sell_stop_price is not None
            if (can_sell and is_order_set(Map.sell)) or (is_stop_set and is_order_set(Map.sell) and (sell_stop_price != trade[Map.sell][Map.stop])):
...
>>>>>>> Solomon-v5.3.1.12.1
=======
            elif is_stop_set and (not is_order_set(Map.sell)):
