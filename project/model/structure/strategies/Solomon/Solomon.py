...
class Solomon(Strategy):
    ...
    def is_tangent_macd_line_positive(cls, vars_map: Map, broker: Broker, pair: Pair, period: int, marketprices: Map, index: int, line_name: str, macd_params: dict = {}) -> bool:
        ...
        vars_map.put(tangent_macd_line_positive,    Map.condition,  f'is_tangent_macd_line_positive_{line_name}[{index}]_{param_str}_{period_str}')
        vars_map.put(line_name,                     Map.value,      f'is_tangent_macd_line_positive_{line_name}[{index}]_{param_str}_{period_str}')
        vars_map.put(macd_line[index],              Map.value,      f'is_tangent_macd_line_positive_{line_name}[{index}]_{param_str}_{period_str}[{index}]')
        vars_map.put(macd_line[prev_index],         Map.value,      f'is_tangent_macd_line_positive_{line_name}[{index}]_{param_str}_{period_str}[{prev_index}]')
