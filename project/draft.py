...
class Draft(Order, Hand):
    ...
    def download_market_histories() -> None:
        ...
        endtime = int(_MF.date_to_unix('2022-09-01 00:00:00'))
    ...
    def get_stage1_history_dates() -> dict:
        dates = {
            # max_period = 6h | API_max_period = 300
            'January_2021': {
                Map.start: 1602979200,  # October 18, 2020 0:00:00
                Map.end: 1612137600	    # February 1, 2021 0:00:00
            },
            'February_2021': {
                Map.start: 1605657600,  # November 18, 2020 0:00:00
                Map.end: 1614556800	    # March 1, 2021 0:00:00
            },
            'Year_2021': {
                Map.start: 1602979200,  # October 18, 2020 0:00:00
                Map.end: 1640995200	    # January 1, 2022 0:00:00
            },
            'January_February_March_2021': {
                Map.start: 1602979200,  # October 18, 2020 0:00:00
                Map.end: 1617235200     # April 1, 2021 0:00:00
            },
            'April_May_June_2021': {
                Map.start: 1610755200,  # January 16, 2021 0:00:00
                Map.end: 1625097600	    # July 1, 2021 0:00:00
            },
            'January_2022_to_April_2022': {
                Map.start: 1634515200,  # October 18, 2021 0:00:00
                Map.end: 1651363200	    # May 1, 2022 0:00:00
            },
            'January_2021_to_June_2021': {
                Map.start: 1602979200,  # October 18, 2020 0:00:00
                Map.end: 1625097600	    # July 1, 2021 0:00:00
            },
            'January_2021_to_April_2022': {
                Map.start: 1602979200,  # October 18, 2020 0:00:00
                Map.end: 1651363200     # May 1, 2022 0:00:00
            },
            'May_to_July_2022': {
                Map.start: 1644883200,  # February 15, 2022 0:00:00
                Map.end: 1656633600     # July 1, 2022 0:00:00
            },
            'January_2021_June_2022': {
                Map.start: 1602979200,  # October 18, 2020 0:00:00
                Map.end: 1656633600     # July 1, 2022 0:00:00
            }
        }
    ...
    def main(cls) -> None:
        ...
        execution = executions[executions_list[view.menu(
            "Choose the execution:", executions_list)]]
        execution()


if __name__ == '__main__':
    boolean_choices = ['False', 'True']
    change_session_id = eval(boolean_choices[Draft.get_view().menu(
        "Do you want to change session's ID:", boolean_choices)])
    if change_session_id:
        session_id = Draft.update_session_id(input("Enter session ID:\n"))
    else:
    ...
    Config.update(Config.STAGE_MODE, Config.STAGE_2)
    print(_MF.prefix() + '\033[46m' +
          f"Start execution: '{session_id}'" + '\033[0m')
    Draft.main()
    Draft.get_broker().close()
    _main_endtime = _MF.get_timestamp()
    print(_MF.prefix(
    ) + '\033[46m' + f"End execution: {_MF.delta_time(_main_starttime, _main_endtime)}" + '\033[0m')
